...
export default class QRScannerView extends Component {
    ...
    barcodeReceived(e) {
        if (e.data != this.state.barcode && !this.props.isLoading){
            ...
            StorageUtil.get("auth").then((value) => {
                ...
                formData.append("auth_code", e.data);
                ...
                HttpUtil.post(API.qrScannerText, formData, {
                    ...
                    if (json && json.code===0 && json.data) {
                        ...
                        TimerUtil.refreshOrderTimer = setInterval(function () {
                            //调用后台接口，通过订单号获取订单状态
                            HttpUtil.get(API.selectByOrderNo + json.data, null, {
                                'Authorization': 'Bearer ' + value.access_token
                            }).then((json) => {
                                if (json.code === 0) {
    ...
    render() {
        return (
            <View style={{flex: 1}}>
                <NavigationBarL
                    title='扫一扫收款'
                    style={{backgroundColor:'rgba(0, 0, 0,0.8)',opacity:0.9}}

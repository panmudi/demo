...
import HttpUtil from "../../util/HttpUtil"; //导入上传图片组件
...
export default class OpenLicense extends Component{
    ...
    componentDidMount() {
        StorageUtil.get("openLicenceInfo").then((value) => {
            if (value) {
                this.setState({
                    photoSource: {
        ...
        return <TouchableOpacity
            ...
            onPress={()=>{
                StorageUtil.get("auth").then((value) => {
                    ...
                    let file = {uri: this.state.photoSource.uri, type: 'application/octet-stream', name: 'openLicence.jpg'};
                    formData.append('accountPermitsImageFile', file);
                    HttpUtil.post(API.updateAccountPermitsImage, formData, {
                        'Authorization': 'Bearer ' + value.access_token,
                        'Content-Type': 'multipart/form-data;charset=utf-8'
                    }).then((json) => {
                        if (json.code === 0) {
                            StorageUtil.save("openLicenceInfo", json.data.bizLicenceImage, () => {
                                NavigatorUtil.goBack(this.props.navigation);
                            })
                        }
                    }, (error) => {
            ...
            <View style={styles.container}>
                ...
                <View style={{backgroundColor:'#fff',borderBottomWidth:scaleSize(1),borderBottomColor:'#eee'}}>
                    <View style={{paddingHorizontal:scaleSize(30)}}>
                        <Text style={{fontSize:scaleSize(28),marginTop:scaleSize(15)}}>开户许可证照片</Text>
                        <View style={[styles.photoBlock,{marginBottom:scaleSize(15)}]}>
                            <View style={styles.photoBox}>
                                <TouchableOpacity onPress={this.selectPhotoTappedAdd.bind(this)}>
                                    <View>
                                        { this.state.photoSource === null ?   <Image style={styles.photoImage} source={require('../../../res/images/addPhoto.png')} />  :
                                            <Image style={styles.photoImage}  source={this.state.photoSource} />
                                        }
                                    </View>
                                </TouchableOpacity>
                                <Text style={{fontSize:scaleSize(20),color:'#888',textAlign:'center',}}>[正面]</Text>
                            </View>
                            <View style={styles.photoBox}>
                                <Image style={styles.photoImage} source={require('../../../res/images/apply/apply_07.png')} />
                                <Text  style={{fontSize:scaleSize(20),color:'#888',textAlign:'center',}}>示意图</Text>
                            </View>
                        </View>
                    </View>
                </View>
